...
test.describe("Kanban Drag & Drop", () => {
  test("kanban page shows columns with tickets", async ({ page }) => {
    ...
    const t1 = await page
      .getByText("Ticket in attesa")
      .isVisible()
      .catch(() => false);
    const t2 = await page
      .getByText("Ticket in lavorazione")
      .isVisible()
      .catch(() => false);
    const t3 = await page
      .getByText("Ticket completato")
      .isVisible()
      .catch(() => false);
    const hasColumns = await page
      .getByText(/in attesa|lavorazione|completato/i)
      .first()
      .isVisible()
      .catch(() => false);
