  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (active)
          toast.error(errorMessage(error, t("errors.loadClients", "Errore caricamento clienti")));
      } finally {
    ...
    <div className="flex flex-col gap-4">
      <div className="pc-card">
        ...
        <div className="pc-card-body grid gap-3 md:grid-cols-4">
          <BundleStat
            label={t("stats.activeBundles", "Bundle attivi")}
            value={stats.activeBundles}
          />
          <BundleStat
            label={t("stats.activeAssignments", "Assegnazioni attive")}
        ...
        <div className="pc-card">
          <div className="pc-card-hd">
            <div className="pc-card-title">
              {bundleManager.editing
                ? t("editBundle", "Modifica bundle")
                : t("newBundle", "Nuovo bundle")}
            </div>
          </div>
      ...
      {(assignmentManager.creating || assignmentManager.editing) && activeTab === "assignments" && (
        <div className="pc-card">
          <div className="pc-card-hd">
            <div className="pc-card-title">
              {assignmentManager.editing
                ? t("editAssignment", "Modifica assegnazione")
                : t("assignBundle", "Assegna bundle a cliente")}
            </div>
          </div>
          <div className="pc-card-body">
            <AssignmentForm
        ...
        <>
          ...
          {activeTab === "assignments" && (
            <AssignmentsTab
              ...
              hasHidden={
                !showAllAssignments && filteredAssignments.length === 0 && assignments.length > 0
              }
            />
          )}
    ...
    <div className="grid gap-3 xl:grid-cols-2">
      {bundles.map((bundle) => (
        <div key={bundle.id} className="pc-card">
            ...
            {canManageBundles && (
              <div className="flex gap-2">
                ...
                <button className="pc-btn pc-btn-ghost pc-btn-sm" onClick={() => onToggle(bundle)}>
                  ...
                  {bundle.active
                    ? t("catalog.deactivate", "Disattiva")
                    : t("catalog.reactivate", "Riattiva")}
                </button>
              </div>
            )}
          </div>
          <div className="pc-card-body grid gap-3 md:grid-cols-4">
            <Metric
              label={t("catalog.fee", "Canone")}
              value={formatBundleMoney(bundle.fee, bundle.currency)}
            />
            <Metric
              label={t("catalog.type", "Tipo")}
              value={BILLING_TYPE_LABEL[bundle.billing_type]}
            />
            <Metric
              label={t("catalog.includedHours", "Ore incluse")}
              value={formatBundleHours(bundle.included_hours)}
            />
            <Metric
            ...
            />
            <Metric
              label={t("catalog.slaResponse", "SLA risposta")}
              value={`${bundle.sla_response_hours} h`}
            />
            <Metric
              label={t("catalog.slaResolution", "SLA risoluzione")}
              value={`${bundle.sla_resolution_hours} h`}
            />
            <Metric
              label={t("catalog.onsite", "On-site")}
              value={formatBundleVisits(bundle.included_onsite_visits)}
            />
            <Metric
              label={t("catalog.remoteSupport", "Supporto remoto")}
              value={
                bundle.remote_support ? t("catalog.included", "Incluso") : t("catalog.no", "No")
              }
            />
          </div>
        </div>
    ...
    <div className="pc-card overflow-hidden">
      <div className="pc-card-hd">
        ...
        <button
          ...
          title={
            showAll
              ? t("assignments.showActiveOnly", "Mostra solo attivi")
              : t("assignments.showAll", "Mostra tutti")
          }
        >
          <Filter className="size-3" />
          {showAll
            ? t("assignments.showActiveOnly", "Solo attivi")
            : t("assignments.showAll", "Mostra tutti")}
        </button>
      </div>
        ...
        {assignments.map((assignment) => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <span className="text-text3">{t("assignments.table.period", "Periodo")}:</span>{" "}
                <span className="font-mono">
                  {assignment.start_date} →{" "}
                  {assignment.end_date ?? t("assignments.noExpiry", "senza scadenza")}
                </span>
              </div>
              <div>
                <span className="text-text3">{t("assignments.table.renewal", "Rinnovo")}:</span>{" "}
                <span>
                  {assignment.auto_renew
                    ? t("assignments.autoRenew", "Automatico")
                    : t("assignments.manualRenew", "Manuale")}
                </span>
              </div>
              <div>
                <span className="text-text3">{t("assignments.table.fee", "Canone")}:</span>{" "}
                <span className="font-mono">
                  {formatBundleMoney(
              ...
              {assignment.notes && (
                <div className="col-span-2">
                  <span className="text-text3">{t("assignments.table.notes", "Note")}:</span>{" "}
                  <span className="text-text2">{assignment.notes}</span>
                </div>
              )}
            </div>
            {canManage && (
        ...
        <OverflowTable>
          <table className="w-full min-w-[980px] text-[12.5px]">
            <thead style={{ background: "var(--surface2)" }}>
              <tr>
                {[
                  t("assignments.table.client", "Cliente"),
                  t("assignments.table.bundle", "Bundle"),
                  t("assignments.table.status", "Stato"),
                  t("assignments.table.period", "Periodo"),
                  t("assignments.table.renewal", "Rinnovo"),
                  t("assignments.table.fee", "Canone"),
                  t("assignments.table.notes", "Note"),
                  t("assignments.table.actions", "Azioni"),
                ].map((h) => (
                  <th
                    key={h}
          ...
          <OverflowTable>
            <table className="w-full min-w-[1060px] text-[12.5px]">
              <thead style={{ background: "var(--surface2)" }}>
                <tr>
                  {[
                    t("usage.table.client", "Cliente"),
                    t("usage.table.bundle", "Bundle"),
                    t("usage.table.hours", "Ore"),
                    t("usage.table.onsite", "On-site"),
                    t("usage.table.extra", "Extra"),
                    t("usage.table.expiry", "Scadenza"),
                    t("usage.table.alert", "Alert"),
                  ].map((h) => (
                    <th
                      key={h}
                      className="px-3 py-2 text-left text-[10.5px] font-bold uppercase text-text3"
...
function BillingTab({
  ...
  function validatePaymentDraft(): boolean {
    ...
    if (!paymentDraft.client_bundle_assignment_id) {
      errs.client_bundle_assignment_id = t(
        "billing.validation.assignmentRequired",
    ...
    <div className="grid gap-4 xl:grid-cols-[420px_1fr]">
      <div className="pc-card">
        ...
        <div className="pc-card-body space-y-3">
          <Metric
            label={t("billing.billableExtraHours", "Ore extra fatturabili")}
            value={formatBundleMoney(extraAmount)}
          />
          {canManage && (
      ...
      <div className="pc-card overflow-hidden">
        <div className="pc-card-hd">
          <div className="pc-card-title">
            {t("billing.paymentHistory", "Storico canoni pagati")}
          </div>
        </div>
        <OverflowTable>
          <table className="w-full text-[12.5px]">
            <thead style={{ background: "var(--surface2)" }}>
              <tr>
                {[
                  t("billing.table.client", "Cliente"),
                  t("billing.table.bundle", "Bundle"),
                  t("billing.table.period", "Periodo"),
                  t("billing.table.amount", "Importo"),
                  t("billing.table.status", "Stato"),
                  t("billing.table.actions", "Azioni"),
                ].map((h) => (
                  <th
                    key={h}
                    className="px-3 py-2 text-left text-[10.5px] font-bold uppercase text-text3"
            ...
            <tbody>
              {payments.map((payment) => (
                <tr key={payment.id} className="border-t" style={{ borderColor: "var(--border)" }}>
                  ...
                  <td className="px-3 py-2">
                    {t(`billing.status.${payment.status}`, payment.status)}
                  </td>
                  <td className="px-3 py-2">
                    {canManage && (
                      <button
