...
For mutations and sensitive operations, pcReady uses TanStack Start server functions (`createServerFn`). These execute with access to the Supabase service role key.
...
sequenceDiagram
    ...
    SFn->>Worker: Execute server-side
