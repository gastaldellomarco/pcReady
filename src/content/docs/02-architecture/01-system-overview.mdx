...
pcReady follows a modern single-page application (SPA) architecture backed by Supabase as a Backend-as-a-Service (BaaS). There is no traditional backend server — business logic runs in server functions executed by Cloudflare Workers.
...
| **Server functions** | TanStack Start `createServerFn` → Cloudflare Workers |
| **Build tool** | Vite 7 |
| **Package manager** | Bun |
