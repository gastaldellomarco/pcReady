...
pcReady uses TanStack Start's `createServerFn` to define server-side logic that executes in Cloudflare Workers with access to the Supabase service role key.
...
Server functions run in Cloudflare Workers — not in Supabase Edge Functions. This means:

- Access to Node.js-compatible APIs (via `nodejs_compat` flag in Wrangler)
