...
export function AnalyticsReportPdf({
  ...
  const monthColumns: PdfColumn<DashboardAnalytics["ticketsByMonth"][number]>[] = [
    ...
    { key: "opened", label: "Aperti", width: "25%", mono: true, value: (row) => String(row.opened) },
    { key: "closed", label: "Chiusi", width: "25%", mono: true, value: (row) => String(row.closed) },
    { key: "avg", label: "Tempo medio", width: "25%", mono: true, value: (row) => formatAvgDays(row.avg_days) },
  ];
  ...
  const techColumns: PdfColumn<DashboardAnalytics["technicianKpi"][number]>[] = [
    ...
    { key: "assigned", label: "Assegnati", width: "20%", mono: true, value: (row) => String(row.assigned) },
    { key: "completed", label: "Completati", width: "20%", mono: true, value: (row) => String(row.completed) },
    { key: "avg", label: "Tempo medio", width: "20%", mono: true, value: (row) => formatAvgDays(row.avg_days) },
  ];
      ...
      <BrandedPage title="Report analytics dashboard" meta={periodLabel}>
        <StatStrip
          stats={[
            ...
            { label: "Tempo medio", value: formatAvgDays(analytics.summary.avgDays), color: pdfPalette.warn },
          ]}
        />
