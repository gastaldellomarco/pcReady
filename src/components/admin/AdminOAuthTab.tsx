...
export function AdminOAuthTab() {
  ...
  const wizardSteps = [
    {
      ...
      description: t(
        "oauth.wizard.step1Description",
        "Nome e descrizione leggibili dagli admin e dagli utenti.",
      ),
    },
    {
      title: t("oauth.wizard.step2Title", "Callback"),
      description: t(
        "oauth.wizard.step2Description",
        "URL sicuri a cui PCReady può rimandare l'utente dopo il consenso.",
      ),
    },
    {
      title: t("oauth.wizard.step3Title", "Permessi"),
      description: t(
        "oauth.wizard.step3Description",
        "Ambiti dati che l'app potrà richiedere durante l'autorizzazione.",
      ),
    },
  ];
  ...
  return (
    <TabsContent value="oauth" className="space-y-5">
      <Card>
        ...
        <CardContent>
          <Alert className="mb-4">
            ...
            <AlertDescription>
              {t(
                "oauth.infoAlert.description",
                "Un Client OAuth permette a un'applicazione esterna (per esempio un tool di automazione, un'app mobile o un sistema ERP) di accedere ai dati di PCReady in modo sicuro, senza condividere le password degli utenti. Crea un client solo se stai collegando un'applicazione esterna che deve operare per conto degli utenti che la autorizzano.",
              )}
            </AlertDescription>
          </Alert>
          ...
          <Alert className="mb-4 border-muted bg-muted/40">
            <AlertTitle className="text-foreground">
              {t("oauth.flowAlert.title", "Flusso supportato")}
            </AlertTitle>
            <AlertDescription className="text-muted-foreground">
              {t(
                "oauth.flowAlert.description",
                "PCReady espone il flusso OAuth 2.0 Authorization Code (response_type=code). Gli integratori avviano l'accesso reindirizzando l'utente all'endpoint di autorizzazione, poi scambiano il codice per un token. Il flusso Client Credentials non è supportato per questi client.",
              )}
            </AlertDescription>
          </Alert>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="clientName">
                    {t("oauth.form.nameLabel", "Nome applicazione")}
                  </Label>
                  <Input
                    id="clientName"
                ...
                <div>
                  <Label htmlFor="clientDescription">
                    {t("oauth.form.descriptionLabel", "Descrizione (facoltativa)")}
                  </Label>
                  <Input
                    id="clientDescription"
                    {...oauthForm.register("description")}
                    placeholder={t(
                      "oauth.form.descriptionPlaceholder",
                      "A cosa serve questa integrazione",
                    )}
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    {t(
                      "oauth.form.descriptionHelp",
            ...
            {wizardStep === 1 && (
              <div>
                <Label htmlFor="redirectUris">
                  {t("oauth.form.redirectLabel", "URL di redirect (callback)")}
                </Label>
                <Textarea
                  id="redirectUris"
                  {...oauthForm.register("redirectUrisRaw")}
                  placeholder={t(
                    "oauth.form.redirectPlaceholder",
            ...
            {wizardStep === 2 && (
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1 mb-3">
                  {t(
                    "oauth.form.scopeHelp",
                    "Seleziona cosa l'applicazione potrà chiedere agli utenti durante l'autorizzazione. Ogni voce mostra il nome tecnico del permesso tra parentesi.",
                  )}
                </p>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 border-t pt-4">
              ...
              {wizardStep < wizardSteps.length - 1 ? (
                ...
                <Button type="submit" disabled={createClientBusy || !oauthForm.formState.isValid}>
                  ...
                  {createClientBusy
                    ? t("oauth.wizard.creating", "Creazione...")
                    : t("oauth.wizard.create", "Crea client")}
                </Button>
              )}
            </div>
          </form>
          ...
          {oauthCreated ? (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {t(
                    "oauth.created.description",
                    "{{name}}: usa questi valori nella tua applicazione. Il secret non sarà più mostrato.",
                    { name: oauthCreated.name },
                  )}
                </DialogDescription>
              </DialogHeader>
              ...
              <Alert variant="destructive" className="mt-2">
                <AlertTitle>
                  {t("oauth.created.saveAlertTitle", "Salva subito il Client Secret")}
                </AlertTitle>
                <AlertDescription>
                  {t(
                    "oauth.created.saveAlertDescription",
                    "Il Client Secret è mostrato una sola volta e non sarà recuperabile da PCReady dopo aver chiuso questa finestra. Copialo e conservalo in un gestore segreti o in configurazione sicura prima di proseguire.",
                  )}
                </AlertDescription>
              </Alert>
              ...
              <div className="space-y-3 text-sm">
                <div>
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-medium text-muted-foreground">
                      {t("oauth.created.clientIdLabel", "Client ID")}
                    </span>
                    <Button
                      type="button"
                      variant="ghost"
                ...
                <div>
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-medium text-muted-foreground">
                      {t("oauth.created.clientSecretLabel", "Client Secret")}
                    </span>
                    <Button
                      type="button"
                      variant="ghost"
              ...
              <Collapsible className="rounded-lg border px-3 py-2">
                ...
                <CollapsibleContent className="space-y-3 pb-3 text-xs text-muted-foreground">
                  <p>
                    {t(
                      "oauth.created.step1Text",
                      "1. Reindirizza l'utente (già autenticato su PCReady) verso l'autorizzazione con i parametri in query.",
                    )}{" "}
                    Sostituisci <code className="rounded bg-muted px-1">redirect_uri</code>,{" "}
                  ...
                  <div>
                    <span className="font-medium text-foreground">
                      {t("oauth.created.getLabel", "GET — autorizzazione")}
                    </span>
                  ...
                  <div>
                    <span className="font-medium text-foreground">
                      {t("oauth.created.postLabel", "POST — scambio code → token")}
                    </span>
                    <pre className="mt-1 p-2 rounded-md bg-muted font-mono text-[11px] break-all whitespace-pre-wrap">
          ...
          {rotatedSecret ? (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {t(
                    "oauth.rotated.description",
                    "Il secret precedente non è più valido. Aggiorna subito le integrazioni che usano questo client.",
                  )}
                </DialogDescription>
              </DialogHeader>
              <Alert variant="destructive" className="mt-2">
                ...
                <AlertDescription>
                  {t(
                    "oauth.rotated.alertDescription",
                    "Questo valore non verrà mostrato di nuovo dopo la chiusura della finestra.",
                  )}
                </AlertDescription>
              </Alert>
        ...
        <CardContent>
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-800">
              <strong>{t("oauth.clientList.notaLabel", "Nota:")}</strong>{" "}
              {t(
                "oauth.clientList.notaText",
                "Per la gestione del consenso OAuth degli utenti, vedere",
              )}{" "}
              <Link to="/docs" className="underline hover:no-underline">
          ...
          {loadingClients ? (
            <p className="text-center py-4 text-muted-foreground">
              {t("oauth.clientList.loading", "Caricamento client...")}
            </p>
          ) : (clients ?? []).length === 0 ? (
            <p className="text-center py-4 text-muted-foreground">
              {t("oauth.clientList.empty", "Nessun client registrato")}
            </p>
          ) : (
                  ...
                  <div key={client.clientId} className="border rounded-lg p-4 space-y-3">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="text-xs font-mono text-muted-foreground mt-2 break-all">
                          {t("oauth.clientList.clientIdLabel", "Client ID: {{id}}", {
                            id: client.clientId,
                          })}
                        </p>
                    ...
                    <div>
                      <p className="text-sm font-medium mb-2">
                        {t("oauth.clientList.permissions", "Permessi:")}
                      </p>
      ...
      <Dialog
        open={!!lifecycle.lifecycleOpenFor}
        onOpenChange={(o) => !o && lifecycle.closeLifecycle()}
      >
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto xs:fixed xs:inset-0 xs:m-0 xs:max-w-full xs:h-full xs:rounded-none xs:overflow-y-auto">
          ...
          {lifecycle.lifecycleLoading ? (
            <p className="text-sm text-muted-foreground py-6 text-center">
              {t("oauth.lifecycle.loading", "Caricamento...")}
            </p>
          ) : lifecycle.lifecycleData && lifecycle.lifecycleOpenFor ? (
            <div className="space-y-6 text-sm">
              <div>
                <h4 className="font-semibold mb-2">
                  {t("oauth.lifecycle.consentsTitle", "Consensi ({{count}})", {
                    count: lifecycle.lifecycleData.consents.length,
                  })}
                </h4>
                {lifecycle.lifecycleData.consents.length === 0 ? (
                  <p className="text-muted-foreground text-xs">
                    {t("oauth.lifecycle.consentsEmpty", "Nessun consenso registrato.")}
                  </p>
                ) : (
                      ...
                      <thead className="bg-muted/50 sticky top-0">
                        <tr>
                          <th className="text-left p-2">
                            {t("oauth.lifecycle.colUser", "Utente")}
                          </th>
                          <th className="text-left p-2">
                            {t("oauth.lifecycle.colScope", "Scope")}
                          </th>
                          <th className="text-left p-2">
                            {t("oauth.lifecycle.colGranted", "Concesso")}
                          </th>
                          <th className="text-left p-2">
                            {t("oauth.lifecycle.colRevoked", "Revoca")}
                          </th>
                        </tr>
                      </thead>
                      <tbody>
              ...
              <div>
                <h4 className="font-semibold mb-2">
                  {t("oauth.lifecycle.authCodesTitle", "Codici di autorizzazione (recenti)")}
                </h4>
                {lifecycle.lifecycleData.authorizationEvents.length === 0 ? (
                  <p className="text-muted-foreground text-xs">
                    {t("oauth.lifecycle.authCodesEmpty", "Nessun codice registrato.")}
                  </p>
                ) : (
                  <OverflowTable className="max-h-40 overflow-y-auto">
                    <table className="w-full text-xs">
                      <thead className="bg-muted/50 sticky top-0">
                        <tr>
                          <th className="text-left p-2">
                            {t("oauth.lifecycle.colCreated", "Creato")}
                          </th>
                          <th className="text-left p-2">
                            {t("oauth.lifecycle.colExpiry", "Scadenza")}
                          </th>
                          <th className="text-left p-2">
                            {t("oauth.lifecycle.colStatus", "Stato")}
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {lifecycle.lifecycleData.authorizationEvents.map((row, i) => (
                          <tr key={`${row.createdAt}-${i}`} className="border-t">
                            ...
                            <td className="p-2">
                              {row.redeemed
                                ? t("oauth.lifecycle.redeemed", "Riscattato")
                                : t("oauth.lifecycle.notRedeemed", "Non riscattato")}
                            </td>
                          </tr>
                        ))}
                      </tbody>
              ...
              <div>
                <h4 className="font-semibold mb-2">
                  {t("oauth.lifecycle.adminAuditTitle", "Audit amministrativo")}
                </h4>
                {lifecycle.lifecycleData.adminEvents.length === 0 ? (
                  <p className="text-muted-foreground text-xs">
                    {t("oauth.lifecycle.adminAuditEmpty", "Nessuna voce.")}
                  </p>
                ) : (
      ...
      <DestructiveConfirmDialog
        ...
        description={
          rotateTarget
            ? t(
                "oauth.confirmRotate.description",
                'Ruotando il secret per "{{name}}", il valore attuale smette di funzionare immediatamente. Tutte le integrazioni che usano il vecchio secret falliranno finché non aggiorni la configurazione. Questa azione viene registrata in audit.',
