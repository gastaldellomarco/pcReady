...
export function AppLogo({
  ...
  return (
    ...
    >
      ...
      >
        <rect x="5" y="8" width="38" height="27" rx="6" fill={screen} stroke={primary} strokeWidth="3" />
        <path d="M17 22.5 22.2 27.5 32 17" fill="none" stroke={check} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M20 39h8M16 43h16" fill="none" stroke={primary} strokeWidth="3" strokeLinecap="round" />
      </svg>
      {wordmarkVisible ? (
        <span className="leading-none tracking-tight" style={{ fontFamily: "var(--font-head)" }}>
          ...
          <span className={tone === "mono" ? "font-semibold" : "font-semibold text-primary"}>Ready</span>
        </span>
      ) : null}
    </div>
  );
