 ...
 *
 */
export default function TemplateStep({ selectedTemplateId, onSelectTemplate }: TemplateStepProps) {
  const { t } = useTranslation("automations");

  return (
    <div>
      <h3 className="text-lg font-semibold">{t("templateStep.title", "Scegli un modello")}</h3>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        >
          ...
          >
            <Plus
              className={cn("size-5", selectedTemplateId === null ? "text-accent" : "text-text3")}
            />
          </div>
...
function TemplateCard({ template, isSelected, onClick }: TemplateCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <Icon className={cn("size-5", isSelected ? "text-accent" : "text-text3")} />
      </div>
      <div className="min-w-0 pr-16">
        <div
          className={cn("text-sm font-semibold", isSelected ? "text-accent" : "text-foreground")}
        >
          {template.name}
        </div>
        <div className="mt-0.5 text-xs text-text3 leading-relaxed">{template.description}</div>
      </div>
    </button>
  );
}
