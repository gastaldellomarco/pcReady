...
export default function ConditionsStep({
  ...
  return (
    <div>
            ...
            {c.type === "field_equals" && (
              <>
                <input
                  ...
                  onChange={(e) => update(c.id, { config: { ...c.config, field: e.target.value } })}
                />
                <input
                  className="rounded-md border px-2 py-1"
                  placeholder="value"
                  value={c.config.value}
                  onChange={(e) => update(c.id, { config: { ...c.config, value: e.target.value } })}
                />
              </>
            )}
            <button onClick={() => remove(c.id)} className="text-sm text-rose-600">
              Rimuovi
            </button>
          </div>
        ))}
      ...
      <div className="mt-3">
        <button onClick={addCondition} className="rounded bg-slate-100 px-3 py-1 text-sm">
          Aggiungi condizione
        </button>
      </div>
    </div>
