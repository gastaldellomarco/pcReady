...
export function BundleStatusBadge({ status }: { status: BundleStatus }) {
  return (
    <BundleBadge label={BUNDLE_STATUS_LABEL[status] ?? status} colors={STATUS_COLORS[status]} />
  );
}
...
export function BundlePriorityBadge({ priority }: { priority: BundleTicketPriority }) {
  return (
    <BundleBadge
      label={BUNDLE_PRIORITY_LABEL[priority] ?? priority}
 ...
 *
 */
export function BundleUsageBar({
  used,
  total,
  label,
}: {
    ...
    <div className="space-y-1">
      <div className="flex items-center justify-between gap-3 text-xs">
        ...
        <span className="text-text3">
          {usedValue.toLocaleString("it-IT", { maximumFractionDigits: 2 })} /{" "}
          {totalValue.toLocaleString("it-IT", { maximumFractionDigits: 2 })}
        </span>
      </div>
      <div className="h-2 overflow-hidden rounded-full bg-surface2">
        <div
