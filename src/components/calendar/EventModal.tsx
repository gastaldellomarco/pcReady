...
const RECURRENCE_VALUES: Array<"none" | RecurrenceFrequency> = [
  "none",
  "daily",
  "weekly",
  "monthly",
...
export function EventModal({
  ...
  useEffect(() => {
    ...
    const end = event
      ...
      : (defaultEndDate ?? new Date(baseDate.getTime() + 60 * 60 * 1000));
    ...
    setReminderOffset(
      event?.reminders?.[0]?.offset_minutes ? String(event.reminders[0].offset_minutes) : "",
    );
  ...
  const selectedTickets = useMemo(
    () =>
      ticketIds.map(
        (id) =>
          ticketOptions.find((option) => option.id === id) ??
          event?.tickets?.find((ticket) => ticket.id === id),
      ),
  ...
  function payload(): CreateCalendarEventData {
    return {
      ...
      reminders: reminderOffset
        ? [{ offset_minutes: Number(reminderOffset), channel: reminderChannel }]
        : [],
    };
  }
  ...
  function handleSave() {
    ...
    const callbacks = {
      onSuccess: () => {
        toast.success(
          isEdit
            ? t("toasts.eventUpdated", "Evento aggiornato")
            : t("toasts.eventCreated", "Evento creato"),
        );
        onSaved();
        onClose();
      },
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEdit ? t("modal.editTitle", "Modifica evento") : t("modal.newTitle", "Nuovo evento")}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-1">
          ...
          {event?.is_recurring_instance && (
            <div className="rounded-md border p-3" style={{ borderColor: pcReadyColors.border }}>
              <Label className="mb-2 flex items-center gap-2">
                ...
                {t("modal.recurringEditScope", "Modifica evento ricorrente")}
              </Label>
              <div className="grid grid-cols-2 gap-2">
                <Button
                  type="button"
                  variant={editScope === "single" ? "default" : "outline"}
                  size="sm"
                  onClick={() => setEditScope("single")}
                >
                  {t("modal.editSingle", "Solo questa data")}
                </Button>
                <Button
                  type="button"
                ...
                >
                  {t("modal.editSeries", "Tutta la serie")}
                </Button>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {!allDay && (
              <div className="grid gap-1.5">
                <Label>{t("modal.startTimeLabel", "Ora inizio")}</Label>
                <Input
                  type="time"
                  value={startTime}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {!allDay && (
              <div className="grid gap-1.5">
                <Label>{t("modal.endTimeLabel", "Ora fine")}</Label>
                <Input
                  type="time"
                  value={endTime}
          ...
          <div className="grid gap-2">
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder={t("modal.ticketSearchPlaceholder", "Cerca ticket...")}
                disabled={!canEdit || isPending}
              />
              {ticketIds.length > 0 && (
                <Button
            ...
            <div
              className="max-h-28 overflow-y-auto rounded-md border"
              ...
              {ticketOptions.map((ticket) => (
                ...
                >
                  <span
                    className="h-2 w-2 rounded-full"
                    style={{
                      background: ticketIds.includes(ticket.id)
                        ? pcReadyColors.primary
                        : pcReadyColors.border,
                    }}
                  />
                  <span className="font-mono text-xs">{ticket.ticket_code}</span>
                  <span className="truncate text-xs" style={{ color: pcReadyColors.textSecondary }}>
                    {ticket.client}
                  </span>
                </button>
              ))}
          ...
          >
            <Label className="flex items-center gap-2">
              <Repeat className="size-4" />
              {t("modal.recurrenceLabel", "Ricorrenza")}
            </Label>
            <div className="grid grid-cols-3 gap-3">
              <Select
                value={recurrence}
                onValueChange={(v) => setRecurrence(v as "none" | RecurrenceFrequency)}
                disabled={!canEdit || isPending || !!event?.is_recurring_instance}
              >
                <SelectTrigger aria-label={t("modal.recurrenceLabel", "Ricorrenza")}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
              ...
              <Input
                ...
                disabled={
                  !canEdit || isPending || recurrence === "none" || !!event?.is_recurring_instance
                }
              />
              <Input
                type="date"
          ...
          >
            <Label className="flex items-center gap-2">
              ...
              {t("modal.reminderLabel", "Promemoria")}
            </Label>
            <div className="grid grid-cols-2 gap-3">
              <Select
                value={reminderOffset || "__none__"}
                onValueChange={(v) => setReminderOffset(v === "__none__" ? "" : v)}
                disabled={!canEdit || isPending}
              >
                <SelectTrigger>
                  <SelectValue />
                ...
                <SelectContent>
                  <SelectItem value="__none__">
                    {t("modal.noReminder", "Nessun promemoria")}
                  </SelectItem>
              ...
              <Select
                ...
                onValueChange={(v) => setReminderChannel(v as ReminderChannel)}
                disabled={!canEdit || isPending || !reminderOffset}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
        ...
        <DialogFooter className="flex-wrap gap-2 pt-2">
          ...
          {canEdit && (
            ...
            >
              {isPending
                ? t("modal.savingButton", "Salvataggio...")
                : t("modal.saveButton", "Salva")}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
