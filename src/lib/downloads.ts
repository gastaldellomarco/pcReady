...
export { buildDownloadFileName, csvCell, dateStamp, rowsToCsv };
export type { CsvCell, DownloadExtension };
...
export function downloadBlob(
  blob: Blob,
  fileName: string,
  options: { revokeDelayMs?: number; documentRef?: Document } = {},
) {
...
export function downloadText(
  content: string,
  fileName: string,
  options: { type?: string; revokeDelayMs?: number } = {},
) {
...
export function createZipBlob(files: ZipTextFile[]) {
  ...
  return new Blob([zipBytes.buffer.slice(0)], { type: MIME_TYPES.zip });
}
