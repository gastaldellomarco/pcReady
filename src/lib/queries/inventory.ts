...
export async function fetchAssignedDeviceIds() {
  const { data, error } = await supabase.from('ticket_device_assignments').select('device_id').is('unassigned_at', null);
...
export async function fetchDevicesList(params: DevicesListParams) {
  ...
  let query = supabase
    .from('devices')
    .select('id, serial, model, os, status, client_id, updated_at, assigned_to, client:clients(name)', { count: 'exact' })
  ...
  if (term) query = query.or(`serial.ilike.%${term}%,model.ilike.%${term}%,assigned_to.ilike.%${term}%`);
...
export function useInventoryList(params: DevicesListParams) {
  ...
  ], queryFn: () => fetchDevicesList(params), placeholderData: (previousData) => previousData });
}

export async function fetchDeviceBySerial(serial: string) {
  const { data, error } = await supabase.from('devices').select('id, serial').ilike('serial', serial).maybeSingle();
  if (error) throw error;
  return data ?? null;
}

export async function createDevice(payload: Record<string, any>) {
  const { data, error } = await supabase.from('devices').insert(payload as any).select('id, serial').single();
  if (error) throw error;
  return data;
}
...
export async function createDevicesBulk(payloads: Record<string, any>[]) {
  ...
  const { data, error } = await supabase.from('devices').insert(payloads as any).select('id');
  if (error) throw error;
...
export function useCreateDevice() {
  ...
  return useMutation({ mutationFn: (payload: Record<string, any>) => createDevice(payload), onSuccess: () => qc.invalidateQueries({ queryKey: ['inventory'] }) });
}

export function useCreateDevicesBulk() {
  const qc = useQueryClient();
  return useMutation({ mutationFn: (payloads: Record<string, any>[]) => createDevicesBulk(payloads), onSuccess: () => qc.invalidateQueries({ queryKey: ['inventory'] }) });
}
