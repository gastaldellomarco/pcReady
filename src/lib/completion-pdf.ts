...
export type CompletionPdfTemplate = "customer" | "technical";

export interface TicketChecklistSummary {
  id: string;
  title: string;
  status: string;
  done: number;
 ...
 *
 * @param ticket - Fully-prepared ticket data (all checklist summaries,
 *   notes log, status history already computed by the caller)
 * @param template - "customer" (cliente) or "technical" (report tecnico)
 * @returns PDF buffer ready for upload to Supabase Storage
 */
  ...
  try {
    ...
    function ensureSpace(height: number) {
      ...
      if (consecutivePageBreaks > MAX_CONSECUTIVE_PAGE_BREAKS) {
        console.error(
          `[completion-pdf] Aborting PDF generation: ${consecutivePageBreaks} consecutive page breaks. Ticket: ${ticket.ticket_code}`,
        );
